...
Record a demonstration in **New workflow → Record workflow**, bind field values to parameters, verify outputs, and publish the reviewed capability. In the main workspace conversation, describe the task: local Ollama matches it against published capability metadata. Choose the proposed workflow, answer the assistant’s input questions one at a time, review the values and authorization, and explicitly start replay. The conversation stays visible alongside the browser. Missing matches offer recording. Catalog matching is probabilistic and includes a guard for known opposite effects; it never authorizes execution. Task matching uses the local model; subsequent input questions use the selected capability schema. Type `cancel` to clear a draft request. The existing LLM UI discovery form still uses the address-update contract.
