...
## Request discovery in English
- Chat distinguishes an explicit request to learn from scratch from an ordinary request to use a saved workflow. It collects missing values in the conversation, keeps supplied values when switching an unstarted replay draft to discovery, clears write approval, and waits for confirmation.
- Fifteen catalog/API tests and eleven targeted browser tests passed. These cover discovery despite a saved match, partial inputs, conversation restoration, switching modes, rejection of unsupported discovery, and zero execution before confirmation. An earlier broader browser run had a scroll timing failure; that test passed in the targeted run. TypeScript checking and the production build passed.
- A live check against the updated API used three genuine Groq requests. The model recognized “Learn a new workflow from scratch” despite an existing address capability and correctly extracted C-104, 75 Pine Street, Fremont, and 94538. The workspace counter moved from 22 to 25; no browser run or banking write was started. This verifies conversational preparation, not a new completed discovery run. Both existing user capabilities were preserved.

## Choose how to create a missing workflow
- Ordinary requests with no saved match now ask the user to choose learning or recording, then ask whether to return verified details or only confirm completion. Both questions accept buttons and English replies. The conversation keeps the current question across reloads; capabilities keep the result preference. All declared outputs still have to pass verification.
- Sixty-seven catalog, API, reply, provider/history, and dashboard tests passed. The browser tests include a scripted balance recording started from chat, publication with completion-only replies, and a real model-free replay that honors that preference. These are automated acceptance tests, not evidence of a person operating the handoff.
- TypeScript and production build passed. Screenshots from an actual empty workspace verify both questions at desktop size; the mobile check confirms the choice buttons stay visible after resizing, with no horizontal overflow or browser errors. That check made no model requests and started no banking run.
- Four genuine Groq requests correctly classified English replies for learning, recording, result details, and completion only. The primary workspace counter moved from 25 to 29. No run was started, and the two existing user capabilities were preserved.
